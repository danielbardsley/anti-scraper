...
async function solveChallenge(challenge) {
  ...
  return { nonces, startedAt, finishedAt: Date.now() };
}
...
function sha256HexFallback(message) {
  ...
  for (let offset = 0; offset < messageBytes.length; offset += 64) {
    for (let index = 0; index < 16; index += 1) {
      ...
      schedule[index] = (((messageBytes[base] << 24) | (messageBytes[base + 1] << 16) | (messageBytes[base + 2] << 8) | messageBytes[base + 3])) >>> 0;
    }
    for (let index = 16; index < 64; index += 1) {
      const sigma0 = rightRotate(schedule[index - 15], 7) ^ rightRotate(schedule[index - 15], 18) ^ (schedule[index - 15] >>> 3);
      const sigma1 = rightRotate(schedule[index - 2], 17) ^ rightRotate(schedule[index - 2], 19) ^ (schedule[index - 2] >>> 10);
      schedule[index] = sum32(schedule[index - 16], sigma0, schedule[index - 7], sigma1);
    }
    let a = h0, b = h1, c = h2, d = h3, e = h4, f = h5, g = h6, h = h7;
    for (let index = 0; index < 64; index += 1) {
      const sigma1 = rightRotate(e, 6) ^ rightRotate(e, 11) ^ rightRotate(e, 25);
      const choice = (e & f) ^ (~e & g);
      const temp1 = sum32(h, sigma1, choice, sha256Constants[index], schedule[index]);
      const sigma0 = rightRotate(a, 2) ^ rightRotate(a, 13) ^ rightRotate(a, 22);
      const majority = (a & b) ^ (a & c) ^ (b & c);
      const temp2 = sum32(sigma0, majority);
      h = g; g = f; f = e; e = sum32(d, temp1); d = c; c = b; b = a; a = sum32(temp1, temp2);
    }
    h0 = sum32(h0, a); h1 = sum32(h1, b); h2 = sum32(h2, c); h3 = sum32(h3, d);
    h4 = sum32(h4, e); h5 = sum32(h5, f); h6 = sum32(h6, g); h7 = sum32(h7, h);
  }
